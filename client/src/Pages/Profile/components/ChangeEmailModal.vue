<template>
  <div v-if="isOpen" class="modal-overlay" @click="close">
    <div class="modal-content" @click.stop>
      <h3 class="modal-title">Изменение email</h3>
  ...
  computed: {
    isFormValid() {
      return (
        this.form.newEmail &&
        this.form.password &&
        this.form.newEmail !== this.currentEmail
      );
    }
  },
  methods: {
    async handleSubmit() {
      ...
      if (this.form.newEmail === this.currentEmail) {
        this.error = 'Новый email должен отличаться от текущего';
        return;
      }
      ...
      try {
        ...
        this.success = 'Email успешно изменен!';
        ...
        this.error = error.message || 'Ошибка при изменении email';
      } finally {
